...
"""
==========================================
Probes (:mod:`fklab.io.spikeGLX.readSGLX`)
==========================================

.. currentmodule:: fklab.io.spikeGLX

Develop by SpikeGLX (no module created by them for this)
...
__all__ = ["load_raw_data", "readMeta", "OriginalChans", "ChannelCountsIM"]
...
def load_raw_data(path):
    ...
    path : str
         path to the bin file from spikeGLX
    ...
    raw_data = makeMemMapRaw(binFullPath, meta)
    return raw_data
...
def main():
    ...
    if dataType == "A":
        ...
        plt.subplots()
        for i in range(0, len(chanList)):
            ax = plt.subplot(1, len(chanList), i + 1)
            ax.plot(tDat, convData[i, :])
        plt.show()
        ...
        for i in range(0, len(dLineList)):
            ax = plt.subplot(1, len(dLineList), i + 1)
